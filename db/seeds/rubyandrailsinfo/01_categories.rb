...
tags_data.each_with_index do |tag_data, index|
  begin
    ...
    category = Category.find_or_create_by!(slug: tag_data['slug']) do |c|
      ...
      c.created_at = Imports::Rubyandrailsinfo::Helpers.parse_time(tag_data['created_at'])
      c.updated_at = Imports::Rubyandrailsinfo::Helpers.parse_time(tag_data['updated_at'])
    end

    # Register for taggings import
    Imports::Rubyandrailsinfo::Helpers.register_category(tag_data['id'], category)

    success_count += 1
    Imports::Rubyandrailsinfo::Helpers.progress(index + 1, tags_data.count, "Categories")
